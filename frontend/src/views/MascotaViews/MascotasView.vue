...
export default {
  ...
  setup() {
    ...
    const getMascotas = () => {
      ...
      MascotasService.obtenerMascotasByOwner(user.id).then((mascotasListResponse) => {
    ...
    function onDelete(id) {
      MascotasService.eliminarMascota(id).then(() => {
      ...
      MascotasService.actualizarMascota(mascota._id, mascota)
        .then((response) => {
          if (response.data.respuesta) {
            const index = mascotasList.value.findIndex((mascota) => mascota._id == editionMode.value.id);
            if (index !== -1) {
      ...
      <table class="table">
        ...
        <tbody>
          <tr v-for="(mascota, index) in mascotasList" :key="mascota._id">
            ...
            <td>
              <div class="form-group" v-if="editionMode.id == mascota._id && editionMode.active">
            ...
            <td>
              <div class="form-group" v-if="editionMode.id == mascota._id && editionMode.active">
                <Field name="breed" type="text" id="breed" v-model="formData.data.breed" class="form-control"
            ...
            <td>
              <div class="form-group" v-if="editionMode.id == mascota._id && editionMode.active">
                <Field name="weight" type="text" id="weight" v-model="formData.data.weight" class="form-control"
            ...
            <td>
              <p v-if="editionMode.id != mascota._id"> {{ mascota.vaccinated ? 'Si' : 'No' }} </p>
              <input v-if="editionMode.id == mascota._id" name="vaccinated" type="checkbox" id="vaccinated"
                v-model="formData.data.vaccinated" class="form-check" />
            </td>
            <td>
              <button @click="askForDeletion(mascota._id)" class="btn btn-danger me-2">Eliminar</button>
              <button v-if="!editionMode.active" @click="editMascota(mascota._id, index)"
                class="btn btn-primary me-2">Editar</button>
              <button v-if="editionMode.id == mascota._id && editionMode.active"
                @click="cancelEditMascota(mascota._id, index)" class="btn btn-primary me-2">Cancelar</button>
              <button v-if="editionMode.id == mascota._id && editionMode.active" type="submit"
