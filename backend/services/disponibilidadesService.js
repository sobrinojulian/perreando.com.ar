...
class ServiceDisponibilidad {
  ...
  filtrarPaseadores = async (zona, horario) => {
    ...
    try {
      ...
      let disponibilidad =
        await this.modelDisponibilidad.obtenerDisponibilidades()
      ...
      if (horario != 0 && zona != 0) {
        paseadoresFiltrados = paseadoresFiltrados.concat(
          listaPaseadores.filter(p => p.horario === horario && p.zona === zona)
        )
      } else if (horario != 0) {
        paseadoresFiltrados = paseadoresFiltrados.concat(
          listaPaseadores.filter(p => p.horario === horario)
        )
      } else if (zona != 0) {
        paseadoresFiltrados = paseadoresFiltrados.concat(
          listaPaseadores.filter(p => p.zona === zona)
        )
      } else {
      ...
      console.log(
        'Error en ServiceDisponibilidad.filtrarPaseadores() --> ',
        error
      )
      return []
    }
  ...
  obtenerDisponibilidadesByPaseador = async paseadorId => {
    try {
      const paseadores =
        await this.modelDisponibilidad.obtenerDisponibilidadesByPaseador(
          paseadorId
        )
      return paseadores
    } catch (error) {
      console.log(
        'Error en ServiceDisponibilidad.obtenerDisponibilidadesByPaseador() --> ',
        error
      )
      return []
    }
  ...
  eliminarDisponibilidad = async id => {
    try {
      ...
      const disponibilidades =
        await this.modelDisponibilidad.obtenerDisponibilidadesById(id)
      const estado = disponibilidades.estado

      if (estado == 0) {
        //Solo se elimina si esta Disponible
        const dispoDeleted =
          await this.modelDisponibilidad.eliminarDisponibilidad(id)
        return { ...dispoDeleted, ...{ respuesta: true } }
      } else {
        mensajeError =
          'No puede eliminar la disponibilidad porque reservada para un paseo.'
        console.log(mensajeError)
        return { respuesta: false, error: mensajeError }
      }
    } catch (error) {
      console.log(
        'Error en ServiceDisponibilidad.eliminarDisponibilidad() --> ',
        error
      )
      return {}
    }
  ...
  cargarDisponibilidades = async dispo => {
    try {
      ...
      if (validate.respuesta) {
        ...
        const disponibilidadesPaseador =
          await this.modelDisponibilidad.obtenerDisponibilidadesByPaseador(
            dispo.paseadorId
          )
        ...
        if (dsCount == null) {
          const disponibilidad =
            await this.modelDisponibilidad.cargarDisponibilidad(dispo)
          return { ...disponibilidad, ...{ respuesta: true } }
        } else {
          mensajeError =
            'Ya existe una disponibilidad en el horario ingresado, por favor ingrese otro.'
          console.log(mensajeError)
          return { respuesta: false, error: mensajeError }
        }
      } else {
        mensajeError =
          'Error al grabar la disponibilidad del paseador, valide los datos ingresados.'
        console.log(mensajeError)
        return { respuesta: validate.respuesta, error: mensajeError }
      }
    } catch (error) {
      console.log(
        'Error en ServiceDisponibilidad.cargarDisponibilidades() --> ',
        error
      )
      return {}
    }
