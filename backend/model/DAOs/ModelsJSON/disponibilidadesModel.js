...
class DisponibilidadModel {
  ...
  async writeFile(disponibilidades) {
    await fs.promises.writeFile(
      this.DISPONIBILIDADFILE,
      JSON.stringify(disponibilidades, null, '\t')
    )
  }
  ...
  obtenerDisponibilidades = async paseadorId => {
    ...
    try {
      disponibilidades = JSON.parse(
        await this.readFile(this.DISPONIBILIDADFILE)
      )
    } catch {}
    ...
    try {
      ...
      console.log(
        'Error en DisponibilidadModel.obtenerDisponibilidades() --> ',
        error
      )
      return paseadorId ? {} : []
    }
  ...
  actualizarDisponibilidad = async dispo => {
    ...
    try {
      const index = disponibilidades.findIndex(
        d => d.disponibilidadId === dispo.disponibilidadId
      )
      if (index != -1) {
        //Retorna -1 sino existe el id ingresado
      ...
      console.log(
        'Error en DisponibilidadModel.actualizarDisponibilidad() --> ',
        error
      )
    }
  }
  ...
  eliminarDisponibilidad = async disponibilidadId => {
    ...
    try {
      const index = disponibilidades.findIndex(
        d => d.disponibilidadId == disponibilidadId
      )
      if (index != -1) {
        //Retorna -1 sino existe el id ingresado
      ...
      console.log(
        'Error en DisponibilidadModel.eliminarDisponibilidad() --> ',
        error
      )
    }
  }
  ...
  cargarDisponibilidad = async disponibilidad => {
    ...
    try {
      ...
      console.log(
        'Error en DisponibilidadModel.cargarDisponibilidad() --> ',
        error
      )
      return {}
    }
